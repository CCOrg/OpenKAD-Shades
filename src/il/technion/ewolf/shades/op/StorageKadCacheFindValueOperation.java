 ...
 * 
 */
public class StorageKadCacheFindValueOperation extends FindValueOperation implements CompletionHandler<KadMessage, Node> {
	...
	@Inject
	StorageKadCacheFindValueOperation(@Named("openkad.local.node") final Node localNode,
	...
	public int getNrQueried() {
		//return this.alreadyQueried.size()+ this.querying.size();
		return this.alreadyQueried.size();
	}

	private synchronized Node takeUnqueried() {

		List<Node> knownNodes = new ArrayList<Node>(knownClosestNodes);
		Collections.shuffle(knownNodes);
		for (int i = 0; i < knownNodes.size(); ++i) {
			final Node n = knownNodes.get(i);
		...
		//		int base = rnd.nextInt(knownClosestNodes.size());
		//		for (int i = 0; i < this.knownClosestNodes.size(); ++i) {
		//			final Node n = this.knownClosestNodes.get((i+base)%knownClosestNodes.size());
		//			if (!this.querying.contains(n) && !this.alreadyQueried.contains(n)) {
		//				this.querying.add(n);
		//				return n;
		//			}
